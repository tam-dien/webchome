...
//        ob_start();
        ...
        $this->chain_id = (isset($attr['chain'])) ? absint(intval($attr['chain'])) : null;
        $this->chain_result_btn = (isset($attr['report'])) ? $attr['report'] : null;
    ...
    public function show_quiz($id, $attr){
        ...
        if(is_user_logged_in()){
            if($payment_terms == "onetime"){
                if( isset($_SESSION['ays_quiz_paypal_purchase']) && isset( $_SESSION['ays_quiz_paypal_purchase'][$id] ) ){
                    if($_SESSION['ays_quiz_paypal_purchase'][$id] == true){
                        $paypal_connection = false;
                        if( isset($_SESSION['ays_quiz_paypal_purchased_item']) && isset( $_SESSION['ays_quiz_paypal_purchased_item'][$id] ) ){
                            if( isset( $_SESSION['ays_quiz_paypal_purchased_item'][$id]['order_id'] ) ){
                                $sql = "SELECT status FROM ". $wpdb->prefix ."aysquiz_orders WHERE id=". absint( $_SESSION['ays_quiz_paypal_purchased_item'][$id]['order_id'] );
                                $order_status = $wpdb->get_var( $sql );
                                if( $order_status != 'finished'){
                                    $_SESSION['ays_quiz_paypal_purchased_item'][$id]['status'] = $order_status;
                    ...
                    $_SESSION['ays_quiz_paypal_purchase'][$id] = false;
                    $paypal_connection = true;
                }
            ...
            }elseif( $payment_terms == "subscribtion" ){
                $current_user = wp_get_current_user();
                $current_usermeta = get_user_meta($current_user->data->ID, "quiz_paypal_purchase");
                if($current_usermeta !== false && !empty($current_usermeta)){
                    foreach($current_usermeta as $usermeta){
                    ...
                    if($quiz_paypal_usermeta !== false){
                        if( isset( $quiz_paypal_usermeta['purchaseDate'] ) && Quiz_Maker_Admin::validateDate( $quiz_paypal_usermeta['purchaseDate'], 'Y-m-d H:i:s' ) ){
                            $subscribtion_expires = strtotime( $quiz_paypal_usermeta['purchaseDate'] . ' +' . $paypal_subscribtion_duration . ' ' . $paypal_subscribtion_duration_by );
        ...
        if(is_user_logged_in()){
            if($stripe_payment_terms == "onetime"){
                if( isset($_SESSION['ays_quiz_stripe_purchase']) && isset( $_SESSION['ays_quiz_stripe_purchase'][$id] ) ){
                    if($_SESSION['ays_quiz_stripe_purchase'][$id] == true){
                        $stripe_connection = false;
                    }else{
                        $stripe_connection = true;
                    }
                }else{
                    $_SESSION['ays_quiz_stripe_purchase'][$id] = false;
                    $stripe_connection = true;
                }
    ...
    /*******************************************************************************************************/
        ...
        if($this->chain_id !== null){
            $chain_quiz_button = "<button type='button' class='action-button ays_chain_next_quiz_button ays_display_none'>
        ...
        if($this->chain_result_btn !== null){
            $chain_quiz_see_result_button = "<button type='button' class='action-button ays_chain_see_result_button ays_display_none'>
    ...
    /*******************************************************************************************************/
        ...
        $quiz_styles .= "
            ...
            #ays-quiz-container-" . $id . " #ays_finish_quiz_" . $id . " a[class~=ajax_add_to_cart]{
                ...
                border-radius: " . $buttons_border_radius . ";
                white-space: nowrap;
                border: 1px solid " . $buttons_text_color . ";
                " . $buttons_width_html . "
            }
...
//        var_dump($enable_navigation_bar);
        ...
        if( $enable_navigation_bar ){
            $questions_nav = "<div id='ays-quiz-questions-nav-wrap-" . $id . "' class='ays-quiz-questions-nav-wrap'>";
                ...
                foreach ($arr_questions as $key => $question_id) {
                    $questions_nav .= "<div class='ays-quiz-questions-nav-item'>";
                        $questions_nav .= "<a href='javascript:void(0);' data-id='".$question_id."' class='ays_questions_nav_question'>".($key+1)."</a>";
                    $questions_nav .= "</div>";
                }
    ...
    public function ays_questions_parts($ids, $quiz_id, $options, $per_page){
        ...
        foreach($ids as $key => $id){
            ...
            if (!empty($question)) {
                ...
                $question_title = stripslashes( htmlspecialchars_decode( $question['question'] ) );
...
//                                'score' => $correct_answered_count_cat[$cat_id] . " / " . $quiz_weight_cats[$cat_id],
    ...
    public function ays_rate_the_quiz(){
        ...
        $quiz_id = absint(intval($_REQUEST["quiz_id"]));
        $score = sanitize_text_field( $_REQUEST['rate_score'] );
        ...
        $results = $wpdb->insert(
            ...
            array(
                ...
                'user_phone' => $user_phone,
                'score'      => $score,
                'review'     => isset($_REQUEST['rate_reason']) ? stripcslashes( esc_sql( sanitize_textarea_field( $_REQUEST['rate_reason'] ) ) ) : '',
                'options'    => '',
                'rate_date'  => $rate_date,
            ),
            array(
