...
class Quiz_Maker_Flash_Cards
{
    ...
    public function ays_all_results_html($attr){
        ...
        $question_id_sql = "SELECT id,question_ids FROM {$quizes_table} WHERE id IN ({$ays_quiz_flash_card_id}) GROUP BY id";
        ...
        switch($ays_quiz_question_by) {
            case 'category':
                $sql = "SELECT q.question,q.category_id,q.question_image,q.explanation,a.answer,a.image
                        ...
                        $content .= '<div style="margin-bottom:20px;" class="ays_quiz_flash_card_content" data-index="0">';
                            $content .= '<div class="ays_quiz_flash_card_container_'.$cat_key.'">';
                                ...
                                foreach ($group_by_cat as $key => $result) {
                                    ...
                                    $ays_quiz_answer                = (isset($result['answer']) && $result['answer'] != '') ? stripslashes( sanitize_text_field( $result['answer'] ) ) : '';
                                    ...
                                    $content .= '<div class="ays_quiz_flash_card">';
                                            ...
                                            $content .= '<div class="ays_quiz_fc_qa">';
                                                ...
                                                if($ays_quiz_question_image != ''){
                                                    $content .= '<div class="ays_quiz_fc_img">';
                                                        $content .= '<img src="'.$ays_quiz_question_image.'" width="200" height="200">';
                                                    $content .= '</div>';
                                                }
                                            $content .= '</div>';

                                        $content .= '</div>';
                                        $content .= '<div class="ays_quiz_back">';
                                            $content .= '<div class="ays_quiz_fc_qa">';
                                                $content .= '<div>'.$ays_quiz_answer.'</div>';
                                                if($ays_quiz_answer_image != ''){
                                                    $content .= '<div class="ays_quiz_fc_img">';
                                                        $content .= '<img src="'.$ays_quiz_answer_image.'" width="200" height="200">';
                                                    $content .= '</div>';
                                                }
                                            $content .= '</div>';
                            ...
                            $content .= '</div>';
                            if ($question_count != 1) {
                                $content .= '<div class="ays_quiz_fc_next_btn_content">';
            ...
            default:
                if( !empty( $ays_question_id ) ){
                    foreach ($ays_question_id as $quiz_key => $ays_q_id) {
                        $sql = "SELECT q.question,q.question_image,q.explanation,a.answer,a.image
                                FROM {$answer_table} AS a
                                LEFT JOIN {$question_table} AS q
                                    ON a.question_id = q.id
